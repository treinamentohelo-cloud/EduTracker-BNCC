...
export const ReinforcementGroups: React.FC = () => {
  ...
  return (
    <div className="space-y-10 relative max-w-7xl mx-auto animate-in fade-in duration-500">
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-4xl shadow-2xl flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-300 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
              {activeTab === 'attendance' ? (
                      ...
                      return (
                        <div key={sid} className={`flex items-center justify-between p-6 rounded-[2rem] border-2 transition-all group ${isPresent ? 'bg-indigo-50/50 border-indigo-200' : 'bg-white border-slate-100 hover:border-slate-200'}`}>
                          <div className="flex items-center gap-6">
                            ...
                            <span className={`text-xl font-black leading-none ${isPresent ? 'text-indigo-900' : 'text-slate-800'}`}>{student.name}</span>
                          </div>
                          <button 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-bottom-6 duration-300">
                  {attendanceHistory.length === 0 ? (
                    <div className="col-span-full py-20 text-center border-4 border-dashed border-slate-200 rounded-[2.5rem] text-slate-300">
                      <Clock size={48} className="mx-auto mb-4 opacity-50" />
                      <p className="font-black text-xl">Ainda não há aulas registradas.</p>
                    </div>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-xl font-black text-slate-800 leading-tight mb-1 group-hover:text-indigo-700">
            ...
            <div className="p-10 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row justify-end items-center gap-4">
              <button onClick={() => setIsCallDiaryOpen(false)} className="px-10 py-5 font-black text-slate-500 hover:text-slate-900 transition-colors uppercase text-xs tracking-widest">Fechar</button>
              {activeTab === 'attendance' && (
                <button 
                  onClick={handleSaveAttendance}
                  className="px-14 py-5 bg-indigo-600 text-white rounded-[1.75rem] font-black shadow-2xl hover:bg-indigo-700 transition-all flex items-center gap-3 text-lg"
                >
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-12 duration-300 overflow-hidden">
            ...
            <div className="p-10 overflow-y-auto space-y-10 flex-1 custom-scrollbar">
              <div className="space-y-3">
                ...
                <input 
                  type="text" 
                  className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-blue-100 font-black text-xl" 
                  placeholder="Ex: Alfabetização Nível I"
                  value={newGroupName}
              ...
              <div className="space-y-6">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Composição do Grupo ({selectedStudents.length})</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {students.map(student => (
                    <button
                      ...
                      onClick={() => {
                        setSelectedStudents(prev => 
                          prev.includes(student.id) ? prev.filter(s => s !== student.id) : [...prev, student.id]
                        );
                      }}
                      className={`flex items-center justify-between p-5 rounded-[1.75rem] border-2 transition-all text-left ${
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm transition-colors ${selectedStudents.includes(student.id) ? 'bg-white/20 text-white' : 'bg-slate-50 text-slate-300'}`}>
                          {student.name.charAt(0)}
                        </div>
                      ...
                      {selectedStudents.includes(student.id) && <Check size={20} className="text-white" />}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="p-10 bg-slate-50 border-t border-slate-100 flex justify-end gap-6">
              <button onClick={closeModal} className="px-10 py-4 font-black text-slate-400 uppercase text-xs">Cancelar</button>
              <button onClick={handleCreateOrEditGroup} className="px-14 py-4 bg-blue-600 text-white rounded-[1.75rem] font-black shadow-2xl hover:bg-blue-700 transition-all text-lg flex items-center gap-3">
      ...
      <div className="bg-gradient-to-br from-indigo-700 to-indigo-950 p-12 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center gap-12">
          <div className="flex-1 space-y-6">
            ...
            <p className="text-indigo-100 max-w-2xl text-xl leading-relaxed opacity-90">
              Identificamos alunos que necessitam de intervenção imediata. Ative células de reforço focadas em habilidades específicas da BNCC.
            </p>
          </div>
          <button 
            onClick={() => setIsCreating(true)}
            className="whitespace-nowrap px-14 py-6 bg-white text-indigo-900 font-black rounded-[2.5rem] hover:bg-indigo-50 transition-all shadow-2xl flex items-center justify-center gap-4 text-xl"
          >
            <Plus size={32} />
            Criar Intervenção
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
             {groups.length === 0 ? (
               ...
               <div key={group.id} className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-lg hover:shadow-2xl hover:border-indigo-400 transition-all group relative overflow-hidden flex flex-col">
                  ...
                  <div className="absolute top-8 right-8 flex gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); db.deleteReinforcement(group.id).then(() => setGroups(db.getReinforcements())) }} 
                  ...
                  <span className="text-[10px] font-black px-4 py-1.5 rounded-full bg-indigo-50 text-indigo-700 uppercase tracking-widest w-fit mb-8">{group.discipline}</span>
                  ...
                  <div className="space-y-4 mb-10 text-sm font-black text-slate-500">
                    <div className="flex items-center gap-4"><Clock size={18} className="text-indigo-400" /> {group.schedule}</div>
                    <div className="flex items-center gap-4"><UserPlus size={18} className="text-indigo-400" /> {group.studentIds.length} Alunos Inscritos</div>
                  </div>
        ...
        <div className="space-y-10">
          <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-xl shadow-slate-200/40 sticky top-10">
            <div className="flex items-center gap-4 mb-10">
              <div className="w-14 h-14 bg-rose-100 text-rose-600 rounded-2xl flex items-center justify-center shadow-lg">
                <AlertCircle size={32} />
              </div>
            ...
            <div className="space-y-4">
               {needsRef.length === 0 ? (
                 <p className="text-slate-400 font-bold text-center py-8">Nenhuma pendência crítica.</p>
